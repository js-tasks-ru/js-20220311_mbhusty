export default class RangePicker {
  ...
  getInputs() {
    const from = this.from.toLocaleString("ru-RU", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
    });
    const to = this.to.toLocaleString("ru-RU", {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
    });
  ...
  getDateTemplate(month) {
    ...
    while (firstDate.getMonth() === month.getMonth()) {
      arr.push(
        new Date(month.getFullYear(), month.getMonth(), firstDate.getDate())
      );
  ...
  toggleMonth = (e) => {
    const direction = e.currentTarget.classList.contains(
      "rangepicker__selector-control-left"
    )
      ? -1
      : 1;
  ...
  setNewRange() {
    ...
    [
      ...
      if (
        Date.parse(item.dataset.value) > this.from.getTime() &&
        Date.parse(item.dataset.value) < this.to.getTime()
      ) {
